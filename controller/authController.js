...
const authController = {
    register: async (req,res) => {
        try {
            ...
            await newUser.save();
                ...
                let subject = "Food Order Registration";
                let content = `<div>
                                    <h1>Hello, ${name} Welcome to food order App</h1>
                                    <h5>${email} is Successfuly registered...</h5>
                                    <p>Happy Ordering ..With lovea a qxjk</p>
                                </div>`;
    ...
    profileUpdate: async (req, res) => {
        try{
            ...
            return res.status(200).json({ msg: "user Profile is updated"})
           // res.json(req.body)
        }catch (err) {
            return res.status(500).json({msg: err.message})
        }
